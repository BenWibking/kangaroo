#!/usr/bin/env python3
"""Plot passive scalar flux from scalar_0 as a function of radius."""
...
FLUX_KEY = "passive_scalar_flux_sphere"
...
def _flux_rows_from_single_radius(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if radius_kpc is None:
        ...
        if radii_kpc is None or len(radii_kpc) != 1:
            raise ValueError("Single-radius JSON must contain radius_kpc.")
        radius_kpc = radii_kpc[0]
    return [
        {
...
def _load_scalar_flux(path: Path) -> ScalarFluxData:
    ...
    if flux_rows is None:
        flux_rows = _flux_rows_from_single_radius(data)
    if not flux_rows:
    ...
    if not (
        ...
        raise ValueError(
            "Radius and passive scalar flux arrays have different lengths."
        )
    if not (
        ...
        raise ValueError("Radius and passive scalar flux values must be finite.")
    ...
    return ScalarFluxData(
        ...
        temperature=_load_temperature_scalar_flux(flux_rows, order),
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Plot passive scalar flux from scalar_0 versus radius from a "
            "flux_surface.json file."
        )
    )
    parser.add_argument(
        "input",
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="passive_scalar_flux_vs_radius.png",
        help="Output image path (default: passive_scalar_flux_vs_radius.png).",
    )
    parser.add_argument(
        "--title",
        default="Passive scalar flux through spherical surfaces",
        help="Plot title.",
    )
    parser.add_argument(
        "--linear-y",
    ...
    data = _load_scalar_flux(Path(args.input))
    title = args.title
    if data.time_myr is not None:
        title = f"{title}, t = {data.time_myr:.3f} Myr"

    outputs = _plot_scalar_flux_set(
        data,
        Path(args.output),
        title,
        args.linear_y,
