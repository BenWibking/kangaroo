...
ParticleArrayData PlotfileReader::read_particle_field(const std::string& particle_type,
  ...
  for (const auto& level_counts : species.header.particle_counts) {
    for (const auto count : level_counts) {
      total_count += count;
    }
  }
  ...
  if (kind == Kind::Int) {
    out.dtype = "int64";
    out.bytes.resize(static_cast<size_t>(total_count) * sizeof(std::int64_t));
    auto* dst = reinterpret_cast<std::int64_t*>(out.bytes.data());
    int64_t cursor = 0;
    for (int lev = 0; lev <= species.header.finest_level; ++lev) {
      const int ngrids = static_cast<int>(species.header.particle_counts[static_cast<size_t>(lev)].size());
      for (int g = 0; g < ngrids; ++g) {
        ParticleBlock block = read_particle_block(species.dir, species.header, lev, g);
        for (int i = 0; i < block.count; ++i) {
          const size_t idx = static_cast<size_t>(i) * static_cast<size_t>(block.num_int) +
                             static_cast<size_t>(index);
          dst[cursor++] = block.int_data[idx];
        }
      }
    }
    return out;
  ...
  out.dtype = species.header.is_single ? "float32" : "float64";
  if (species.header.is_single) {
    out.bytes.resize(static_cast<size_t>(total_count) * sizeof(float));
      ...
      for (int g = 0; g < ngrids; ++g) {
        ...
        for (int i = 0; i < block.count; ++i) {
          const size_t idx = static_cast<size_t>(i) * static_cast<size_t>(block.num_real) +
                             static_cast<size_t>(index);
          dst[cursor++] = block.real_data_single[idx];
        }
      }
    }
  } else {
    out.bytes.resize(static_cast<size_t>(total_count) * sizeof(double));
    ...
    for (int lev = 0; lev <= species.header.finest_level; ++lev) {
      ...
      for (int g = 0; g < ngrids; ++g) {
        ...
        for (int i = 0; i < block.count; ++i) {
          const size_t idx = static_cast<size_t>(i) * static_cast<size_t>(block.num_real) +
                             static_cast<size_t>(index);
          dst[cursor++] = block.real_data_double[idx];
        }
      }
    }
  }
